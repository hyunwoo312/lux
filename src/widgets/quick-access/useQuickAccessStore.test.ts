...
describe("useQuickAccessStore", () => {
  ...
  it("keeps a pin recoverable when it is unpinned from a browser list", () => {
  ...
  it("forgets the removal once it is dismissed", () => {
    ...
    const id = links(ID)[0]!.id;
    store().removeLink(ID, id);
    store().dismissRemoved(ID, id);
    store().undoRemove(ID);

    expect(links(ID)).toHaveLength(0);
  });

  it("undoes the removal the toast names when a pin is unpinned while a toast is live", () => {
    store().addLink(ID, "One", "one.com");
    store().addLink(ID, "Two", "two.com");

    store().removeLink(ID, links(ID)[0]!.id);
    store().togglePin(ID, "Two", "two.com");

    expect(useToastStore.getState().toast?.message).toBe("Removed Two");
    useToastStore.getState().runAction();

    expect(links(ID).map((link) => link.title)).toEqual(["Two"]);
  });
